...
name: meeting-sync
description: >
  Pull Outlook meetings into the Promega Project Planner Meetings/ mount via Softserve MS365.
...
Pulls Outlook calendar events via `mcp__softserve__*` (Microsoft Graph `get-calendar-view`) and reconciles them with the engineer's workspace. Every run covers **Monday–Friday of the current workweek + Monday–Friday of next workweek**.
...
## Prerequisite — Softserve MCP

**Interactive:** if not connected, offer setup via Claude Customize → Connectors → Softserve. Retry after auth.

**Scheduled:** log `"Softserve MCP not connected — scheduled sync skipped."` and exit.
...
```
get-calendar-view(startDateTime=START_TS, endDateTime=END_TS, $top=50)
```
...
```
get-calendar-event(seriesMasterId) → .recurrence.{pattern, range}
```
...
For queued meetings, follow `TRANSCRIPT_INGEST.md` (sibling file): section A (T1–T8, transcript via `list-meeting-transcripts`, match by `createdDateTime`) and section B (recording link → `| Recording | <url> |` in Meeting Details).
...
- **Softserve disconnect mid-sync:** skip affected meeting, log, continue — don't abort.
