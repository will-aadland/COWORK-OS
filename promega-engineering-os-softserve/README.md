...
> **This is the Softserve variant.** It's identical to the standard `promega-engineering-os` plugin except `meeting-sync` is rewired onto the Promega-internal **Softserve MS365 connector** (Microsoft Graph). You get a three-column Attendees table (Name, Email, Response) populated with the display names Softserve returns, Recurrence and Recording rows in `## Meeting Details`, and simpler transcript matching. **Requires the Softserve connector enabled in Claude Customize.** If you don't have Softserve access, install the standard `promega-engineering-os` plugin from the same marketplace instead — workspace files are forward-compatible between the two variants.
...
**Meeting ingest.** The `meeting-sync` skill pulls Outlook events into the `Meetings/` mount via the **Softserve MS365 connector** (Microsoft Graph). It uses the planner's em-dash folder naming, populates Meeting Details (Date, Time, Location, Organizer, Importance, MeetingId, LastSynced, Recurrence, Recording), a three-column Attendees table (Name, Email, Response) with display names from Softserve, and Agenda. Post-meeting it pulls transcript summaries into `## Transcript Summary` and recording links into Meeting Details. Every run covers the current workweek plus next workweek (Mon–Fri, 2 weeks). Two modes: scheduled (auto-pulls last-workday transcripts + recordings, deletes cancelled-meeting folders silently) and interactive (asks per cancelled folder, asks what to pull). OOO events update a single `Meetings/OOO.md` file rather than creating folders. **`meeting-sync` does not run automatically during `/begin`; trigger it manually with "sync my meetings" when you're ready.**
...
| Skill | What it does |
|---|---|
| `meeting-sync` | Pulls Outlook events for this workweek plus next workweek into `Meetings/` via the **Softserve MS365 connector** (Microsoft Graph). Populates rich Meeting Details (incl. Recurrence row from the series master, Recording URL when available) and a three-column Attendees table (Name, Email, Response) with display names from Softserve. Two modes: scheduled (auto-pulls last-workday transcripts + recordings, deletes cancelled folders silently) and interactive (asks per cancelled folder, asks what to pull). OOO events update `Meetings/OOO.md` rather than creating folders. Does **not** create `Chat Summaries/` inside meeting folders. |
