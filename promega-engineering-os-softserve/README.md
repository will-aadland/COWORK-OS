# Promega Engineering OS — Softserve variant

> **This is the Softserve variant.** It's identical to the standard `promega-engineering-os` plugin except `meeting-sync` is rewired onto the Promega-internal **Softserve MS365 connector** (Microsoft Graph). You get a four-column Attendees table (Email, Response, Attended, Minutes), Recurrence and Recording rows in `## Meeting Details`, and simpler transcript matching. **Requires the Softserve connector enabled in Claude Customize.** If you don't have Softserve access, install the standard `promega-engineering-os` plugin from the same marketplace instead — workspace files are forward-compatible between the two variants.
...
**Meeting ingest.** The `meeting-sync` skill pulls Outlook events into the `Meetings/` mount via the **Softserve MS365 connector** (Microsoft Graph). It uses the planner's em-dash folder naming, populates Meeting Details (Date, Time, Location, Organizer, Importance, MeetingId, LastSynced, Recurrence, Recording), the enriched Attendees table (Email, Response, Attended, Minutes), and Agenda. Post-meeting it pulls transcript summaries into `## Transcript Summary`, attendance reports into the Attendees table, and recording links into Meeting Details. Every run covers the current workweek plus next workweek (Mon–Fri, 2 weeks). Two modes: scheduled (auto-pulls last-workday post-meeting data, deletes cancelled-meeting folders silently) and interactive (asks per cancelled folder, asks what to pull). OOO events update a single `Meetings/OOO.md` file rather than creating folders. **`meeting-sync` does not run automatically during `/begin`; trigger it manually with "sync my meetings" when you're ready.**
...
| Skill | What it does |
|---|---|
| `meeting-sync` | Pulls Outlook events for this workweek plus next workweek into `Meetings/` via the **Softserve MS365 connector** (Microsoft Graph). Populates rich Meeting Details (incl. Recurrence row from the series master, Recording URL when available) and the four-column Attendees table (Email, Response, Attended, Minutes). Two modes: scheduled (auto-pulls last-workday transcripts + attendance + recordings, deletes cancelled folders silently) and interactive (asks per cancelled folder, asks what to pull). OOO events update `Meetings/OOO.md` rather than creating folders. Does **not** create `Chat Summaries/` inside meeting folders. |
...
1. **Softserve MS365 connector**: enable in Claude Cowork (Customize → Connectors → Softserve) — required by `meeting-sync` for calendar enumeration (`get-calendar-view`), transcript ingest (`list-meeting-transcripts`, `get-meeting-transcript-content`), attendance reports (`list-meeting-attendance-records`), and recording links (`list-meeting-recordings`). Also enables `get-current-user` for `/join-team` name resolution. All required tools are read-only and auto-allowed.
2. **Microsoft / SharePoint search connector** (Anthropic-default Outlook MCP): enable for the `sharepoint-search` skill's full-text search (`sharepoint_search`, `chat_message_search`, `outlook_email_search`). `meeting-sync` no longer depends on this connector — only `sharepoint-search` does.
3. **SharePoint sites** the plugin commonly references during onboarding and search. The list below is a starting point, the engineer can add or remove during `/begin`:
   - `RDC Renovations`
