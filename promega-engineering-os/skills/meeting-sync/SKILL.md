...
**Also detect the nested-directory bug.** If a folder under `Meetings/` contains a child directory other than the standard three (`Notes`, `Files`, `Transcripts`), it's likely a `/` in the original title that older skill versions used to create a path. The parent + child name concatenated (with space) is the actual canonical title. Treat this as a legacy variant to migrate.

**Migrate legacy `Chat Summaries/` folders inside meetings.** Older versions of this skill created `Chat Summaries/` inside meeting folders. The planner ignores it there (see PLANNER_REFERENCE.md Section 4.3). On any sync, if a meeting folder contains a `Chat Summaries/` subfolder:
...
**Merge logic** (same as the colon migration):
...
**Permission notes for this sandbox:**
...
**Log every migration** in the final report so the engineer knows what happened. Use a format like:
```
...
**Legacy table format migration.** Some workspaces have an older OOO.md that used a markdown table (`| Date | Who |` columns). On first run with the new skill, detect the table format and migrate every existing row to the `## Entries` list format. Pattern:
